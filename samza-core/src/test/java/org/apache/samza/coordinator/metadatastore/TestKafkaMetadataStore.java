...
//    kafkaMetadataStore = new KafkaMetadataStore(new MapConfig(), streamManagerMock, localityManagerMock);
    containerContext = new SamzaContainerContext("0", new MapConfig(), new ArrayList<>(), new MetricsRegistryMap());
    kafkaMetadataStore.init(containerContext);
  }
...
//    Assert.assertEquals(expectedTaskNameLocality, kafkaMetadataStore.readTaskLocality());
  ...
  public void testDeleteTaskLocality() {
    Mockito.doNothing()
           ...
           .deleteTaskMappings(Mockito.any());

//    kafkaMetadataStore.deleteTaskLocality(ImmutableList.of(new TaskName("task-1"), new TaskName("task-2")));

    Mockito.verify(taskAssignmentManagerMock, Mockito.atLeastOnce()).deleteTaskMappings(Mockito.any());
  }
  ...
  public void testWriteTaskLocality() {
    Mockito.doNothing()
           ...
           .writeContainerToHostMapping(containerContext.id, TEST_PROCESSOR_HOST1, "", "");

//    kafkaMetadataStore.writeTaskLocality(new ArrayList<>(), new LocationId(TEST_PROCESSOR_HOST1));
