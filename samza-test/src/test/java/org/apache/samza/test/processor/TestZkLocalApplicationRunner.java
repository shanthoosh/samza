...
@PrepareForTest(ZkBarrierForVersionUpgrade.class)
public class TestZkLocalApplicationRunner extends StandaloneIntegrationTestHarness {
  ...
  @Test
  public void testStreamAppShutDownDuringWaitForBarrierShouldReTriggerBarrier() throws Exception {
    ...
    StreamApplication streamApp1 = new TestStreamApplication(inputKafkaTopic, outputKafkaTopic, processedMessagesLatch1, null);
    StreamApplication streamApp2 = new TestStreamApplication(inputKafkaTopic, outputKafkaTopic, processedMessagesLatch2, null);
    ZkBarrierProxy zkBarrierProxy = new ZkBarrierProxy(zkKeyBuilder.getJobModelVersionBarrierPrefix(), zkUtils, null, () -> applicationRunner2.kill(streamApp2));
    PowerMockito.whenNew(ZkBarrierForVersionUpgrade.class).withAnyArguments().thenReturn(zkBarrierProxy);
    ...
    PowerMockito.verifyNew(ZkBarrierForVersionUpgrade.class);
  }

  @Test
  public void testZookeeperFailureDuringWaitForBarrierShouldStopStreamApplication() throws Exception {
    ZkBarrierProxy zkBarrierProxy = new ZkBarrierProxy(zkKeyBuilder.getJobModelVersionBarrierPrefix(), zkUtils, null, () -> zookeeper().shutdown());
    PowerMockito.whenNew(ZkBarrierForVersionUpgrade.class).withAnyArguments().thenReturn(zkBarrierProxy);
  ...
  private static class ZkBarrierProxy extends ZkBarrierForVersionUpgrade {

    private Runnable runnable;

    public ZkBarrierProxy(String barrierRoot, ZkUtils zkUtils, ZkBarrierListener barrierListener, Runnable runnable) {
      super(barrierRoot, zkUtils, barrierListener);
    ...
    public void create(String version, List<String> participants) {
      runnable.run();
      super.create(version, participants);
    }

    @Override
    public void join(String version, String participantId) {
      runnable.run();
      super.join(version, participantId);
    }

    @Override
    public void expire(String version) {
      runnable.run();
      super.expire(version);
    }
  }

  interface StreamApplicationCallback {
